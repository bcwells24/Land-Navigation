...
def calculate_path_distance_with_pickup(path, min_distance_threshold, start_points, points):
    total = 0
    # Check every segment except the final one.
...
def generate_paths(points, min_distance_threshold, max_total_distance):
    ...
    seen_first_two = set()
    for s in start_points:
        for path in itertools.permutations(num_points, 3):
            full_path = (s,) + path
